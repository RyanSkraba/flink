 ...
 *
 * This class contains types of [[org.apache.flink.api.common.typeinfo.Types]] and adds types for
 * Scala specific classes (such as [[Unit]] or case classes).
 *
 * In many cases, Flink tries to analyze generic signatures of functions to determine return types
 * automatically. This class is intended for cases where type information has to be supplied
...
object Types {
  ...
  /** Returns type information for [[java.time.LocalDate]]. Supports a null value. */
  val LOCAL_DATE: TypeInformation[java.time.LocalDate] = JTypes.LOCAL_DATE

  /** Returns type information for [[java.time.LocalTime]]. Supports a null value. */
  val LOCAL_TIME: TypeInformation[java.time.LocalTime] = JTypes.LOCAL_TIME

  /** Returns type information for [[java.time.LocalDateTime]]. Supports a null value. */
  val LOCAL_DATE_TIME: TypeInformation[java.time.LocalDateTime] = JTypes.LOCAL_DATE_TIME
  ...
  /**
   * Returns type information for [[org.apache.flink.types.Row]] with fields of the given types. A
   * row itself must not be null.
   *
   ...
   * row fields cannot be automatically inferred; therefore, it is required to provide type
   ...
   *
   * This method generates type information with fields of the given types; the fields have the
  ...
  /**
   * Returns type information for [[org.apache.flink.types.Row]] with fields of the given types and
   ...
   * either public (and non-final) or have a public getter and a setter method that follows the Java
   * beans naming conventions for getters and setters.
   *
   * A POJO is a fixed-length, null-aware composite type with non-deterministic field order. Every
   ...
   * information for all fields, an [[org.apache.flink.api.common.functions.InvalidTypesException}]]
   * is thrown. Alternatively, you can use [[Types.POJO(Class, Map)]] to specify all fields
   * manually.
   *
   * @param pojoClass
   ...
   *
   * '''Note:''' In most cases the type information of fields can be determined automatically, we
   * recommend to use [[Types.POJO(Class)]].
   *
   * @param pojoClass
   *   POJO class
   * @param fields
   *   map of fields that map a name to type information. The map key is the name of the field and
  ...
  /**
   * Returns generic type information for any Scala/Java object. The serialization logic will use
   * the general purpose serializer Kryo.
   *
   ...
   * deterministic field order. Fields of a case class are typed. Case classes and tuples are the
   * most efficient composite type; therefore, they do not not support null-valued fields unless the
   * type of the field supports nullability.
   *
   ...
   *
   * A Scala tuple is a fixed-length composite type for storing multiple values in a deterministic
   * field order. Fields of a tuple are typed. Tuples are the most efficient composite type;
   * therefore, they do not not support null-valued fields unless the type of the field supports
   * nullability.
   *
  ...
  /**
   * Returns type information for Scala/Java arrays of primitive type (such as `Array[Byte]`). The
   * array and its elements do not support null values.
   *
   * @param elementType
   ...
   * `Array[java.lang.Integer]`). The array itself must not be null. Null values for elements are
   * supported.
   *
   * @param elementType
  ...
  /**
   * Returns type information for Scala collections that implement [[Traversable]]. Null values are
   * not supported.
   */
