 ...
 * cause execution in the current JVM, a remote environment will cause execution on a remote cluster
 * installation.
 *
...
class ExecutionEnvironment(javaEnv: JavaEnv) {
  ...
  /**
   * Sets the parallelism (parallelism) for operations executed through this environment. Setting a
   * parallelism of x here will cause all operators (such as join, map, reduce) to run with x
   * parallel instances. This value can be overridden by specific operations using
   ...
   * overridden by individual operations using [[DataSet.setParallelism]]
   */
  ...
  /**
   * Creates a DataSet of Strings produced by reading the given file line wise. This method is
   * similar to [[readTextFile]], but it produces a DataSet with mutable [[StringValue]] objects,
   * rather than Java Strings. StringValues can be used to tune implementations to be less object
   * and garbage collection heavy.
   *
   * @param filePath
   *   The path of the file, as a URI (e.g., "file:///some/local/file" or
   *   "hdfs://host:port/file/path").
  ...
  /**
   * Creates a DataSet by reading the given CSV file. The type parameter must be used to specify a
   * Tuple type that has the same number of fields as there are fields in the CSV file. If the
   * number of fields in the CSV file is not the same, the `includedFields` parameter can be used to
   * only read specific fields.
   *
   * @param filePath
   *   The path of the file, as a URI (e.g., "file:///some/local/file" or
   *   "hdfs://host:port/file/path").
  ...
  /**
   * Creates a DataSet that represents the primitive type produced by reading the given file in
   * delimited way.This method is similar to [[readCsvFile]] with single field, but it produces a
   * DataSet not through Tuple. The type parameter must be used to specify the primitive type.
   *
   * @param filePath
   *   The path of the file, as a URI (e.g., "file:///some/local/file" or
   *   "hdfs://host:port/file/path").
   ...
   * via [[org.apache.flink.api.common.functions.RuntimeContext#getDistributedCache]]
   *
   * @param filePath
   ...
   * results [[DataSet.print]], writing results (e.g. [[DataSet.writeAsText]], [[DataSet.write]], or
   * other generic data sinks created with [[DataSet.output]].
   *
   * The program execution will be logged and displayed with a generated default name.
   *
  ...
  /**
   * Creates the program's [[org.apache.flink.api.common.Plan]]. The plan is a description of all
   * data sources, data sinks, and operations and how they interact, as an isolated unit that can be
   * executed with an [[PipelineExecutor]]. Obtaining a plan and starting it with an executor is an
   * alternative way to run a program and is only possible if the program only consists of
...
object ExecutionEnvironment {
  ...
  /**
   * Creates a local execution environment. The local execution environment will run the program in
   * a multi-threaded fashion in the same JVM as the environment was created in. This method allows
   * to pass a custom Configuration to the local environment.
   */
  ...
  /**
   * Creates a [[ExecutionEnvironment]] for local program execution that also starts the web
   * monitoring UI.
   *
   ...
   * is set explicitly via [[ExecutionEnvironment.setParallelism()]].
   *
   * @param host
