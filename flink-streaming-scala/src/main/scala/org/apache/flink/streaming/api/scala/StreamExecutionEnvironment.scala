...
class StreamExecutionEnvironment(javaEnv: JavaEnv) extends AutoCloseable {
  ...
  /**
   * Sets the parallelism for operations executed through this environment. Setting a parallelism of
   * x here will cause all operators (such as join, map, reduce) to run with x parallel instances.
   * This value can be overridden by specific operations using [[DataStream#setParallelism(int)]].
   */
  ...
  /**
   * Sets the runtime execution mode for the application (see [[RuntimeExecutionMode]]). This is
   * equivalent to setting the "execution.runtime-mode" in your application's configuration file.
   *
   * We recommend users to NOT use this method but set the "execution.runtime-mode" using the
   * command-line when submitting the application. Keeping the application code configuration-free
   * allows for more flexibility as the same application will be able to be executed in any
   * execution mode.
   *
   * @param executionMode
  ...
  /**
   * Sets the maximum degree of parallelism defined for the program. The maximum degree of
   * parallelism specifies the upper limit for dynamic scaling. It also defines the number of key
   ...
   * deployed into a shared slot. There's no guarantee that the scheduler always deploy the grouped
   * operators together. In cases grouped operators are deployed into separate slots, the slot
   * resources will be derived from the specified group requirements.
   *
  ...
  /**
   * Returns the default parallelism for this execution environment. Note that this value can be
   * overridden by individual operations using [[DataStream#setParallelism(int)]]
   */
   ...
   * default the output buffers flush frequently to provide low latency and to aid smooth developer
   * experience. Setting the parameter can result in three logical modes:
   *
   * <ul> <li>A positive integer triggers flushing periodically by that integer</li> <li>0 triggers
   * flushing after every record thus minimizing latency</li> <li>-1 triggers flushing only when the
   * output buffer is full thus maximizing throughput</li> </ul>
   */
   ...
   * [[CheckpointingMode.EXACTLY_ONCE]] mode. The state will be stored in the configured state
   * backend.
   *
   * NOTE: Checkpointing iterative streaming dataflows in not properly supported at the moment. For
   * that reason, iterative jobs will not be started if used with enabled checkpointing. To override
   ...
   *
   * An operator can acknowledge a checkpoint as soon as the changes in the log have reached the
   * durable checkpoint storage.
   *
   * The state tables are persisted periodically, independent of the checkpoints. We call this the
   * materialization of the state on the checkpoint storage.
   *
   * Once the state is materialized on checkpoint storage, the state changelog can be truncated to
   * the corresponding point.
   *
   * It establish a way to drastically reduce the checkpoint interval for streaming applications
   * across state backends. For more details please check the FLIP-158.
   *
  ...
  // --------------------------------------------------------------------------------------------
  /**
   * Adds a new Kryo default serializer to the Runtime. <p/> Note that the serializer instance must
   * be serializable (as defined by java.io.Serializable), because it may be distributed to the
  ...
  /**
   * Registers the given type with the serialization stack. If the type is eventually serialized as
   * a POJO, then the type is registered with the POJO serializer. If the type ends up being
   ...
   *
   * If you set the characteristic to IngestionTime of EventTime this will set a default watermark
   * update interval of 200 ms. If this is not applicable for your application you should change it
   * using [[org.apache.flink.api.common.ExecutionConfig#setAutoWatermarkInterval(long)]]
   *
   ...
   *   [[org.apache.flink.api.common.ExecutionConfig#setAutoWatermarkInterval(long]]. If you are
   *   using [[TimeCharacteristic.IngestionTime]], please manually set an appropriate
   *   [[WatermarkStrategy]]. If you are using generic "time window" operations (for example
   *   [[KeyedStream.timeWindow()]] that change behaviour based on the time characteristic, please
   *   use equivalent operations that explicitly specify processing time or event time.
   */
  @deprecated
  @PublicEvolving
   ...
   *
   * @deprecated
   *   Use [[fromSequence(long, long)]] instead to create a new data stream that contains
  ...
  /**
   * Creates a new data stream that contains a sequence of numbers (longs) and is useful for testing
   ...
   * long integer values), you may consider executing the application in a streaming manner because
   * of the end bound that is pretty far away.
   *
   * Use [[fromSource(Source,WatermarkStrategy, String)]] together with [[NumberSequenceSource]] if
   * you required more control over the created sources. For example, if you want to set a
   * [[WatermarkStrategy]].
   */
  ...
  /**
   * Creates a DataStream that contains the given elements. The elements must all be of the same
   * type.
   *
   * Note that this operation will result in a non-parallel data source, i.e. a data source with a
   * parallelism of one.
   */
   ...
   * The character set with the given name will be used to read the files.
   */
  ...
  /**
   * Reads the given file with the given input format. The file path should be passed as a URI
  ...
  /**
   * Creates a DataStream that contains the contents of file created while system watches the given
   * path. The file will be read with the system's default character set. The user can check the
  ...
  def readFileStream(
      ...
      watchType: FileMonitoringFunction.WatchType = FileMonitoringFunction.WatchType.ONLY_NEW_FILES)
      : DataStream[String] =
    asScalaStream(javaEnv.readFileStream(StreamPath, intervalMillis, watchType))

  /**
   * Reads the contents of the user-specified path based on the given [[FileInputFormat]]. Depending
   ...
   * @param watchType
   *   The mode in which the source should operate, i.e. monitor path and react to new data, or
   *   process once and exit
   * @param interval
   *   In the case of periodic path monitoring, this specifies the interval (in millis) between
   *   consecutive path scans
   * @param filter
  ...
  /**
   * Reads the contents of the user-specified path based on the given [[FileInputFormat]]. Depending
   * on the provided [[FileProcessingMode]], the source may periodically monitor (every `interval`
   * ms) the path for new data ([[FileProcessingMode.PROCESS_CONTINUOUSLY]]), or process once the
   * data currently in the path and exit ([[FileProcessingMode.PROCESS_ONCE]]). In addition, if the
   * path contains files not to be processed, the user can specify a custom [[FilePathFilter]]. As a
   * default implementation you can use [[FilePathFilter.createDefaultFilter()]].
   *
   * ** NOTES ON CHECKPOINTING: ** If the `watchType` is set to [[FileProcessingMode#PROCESS_ONCE]],
   ...
   * the downstream [[ContinuousFileReaderOperator readers]] to read the actual data, and exits,
   * without waiting for the readers to finish reading. This implies that no more checkpoint
   * barriers are going to be forwarded after the source exits, thus having no checkpoints after
   * that point.
   *
   * @param inputFormat
   *   The input format used to create the data stream
  ...
  /**
   * Creates a new DataStream that contains the strings received infinitely from socket. Received
   * strings are decoded by the system's default character set. The maximum retry interval is
  ...
  /**
   * Generic method to create an input data stream with a specific input format. Since all data
   * streams need specific information about their types, this method needs to determine the type of
  ...
  /**
   * Create a DataStream using a user defined source function for arbitrary source functionality. By
   * default sources have a parallelism of 1. To enable parallel execution, the user defined source
   ...
   * resulted in a "sink" operation. Sink operations are for example printing results or forwarding
   ...
   * The program execution will be logged and displayed with a generated default name.
   *
   * <b>ATTENTION:</b> The caller of this method is responsible for managing the lifecycle of the
   * returned [[JobClient]]. This means calling [[JobClient#close()]] at the end of its usage. In
   * other case, there may be resource leaks depending on the JobClient implementation.
   *
   * @return
  ...
  /**
   * Getter of the [[org.apache.flink.streaming.api.graph.StreamGraph]] of the streaming job. This
   * call clears previously registered [[org.apache.flink.api.dag.Transformation transformations]].
   *
   * @return
  ...
  /**
   * Getter of the [[org.apache.flink.streaming.api.graph.StreamGraph]] of the streaming job with
   * the option to clear previously registered
   * [[org.apache.flink.api.dag.Transformation transformations]]. Clearing the transformations
   * allows, for example, to not re-execute the same operations when calling [[execute()]] multiple
   * times.
   *
   ...
   *
   * Note that the returned configuration might not be complete. It only contains options that have
   * initialized the environment or options that are not represented in dedicated configuration
   ...
   * local files (which will be distributed via BlobServer), or files in a distributed file system.
...
object StreamExecutionEnvironment {
  ...
  /**
   * Creates an execution environment that represents the context in which the program is currently
   * executed. If the program is invoked standalone, this method returns a local execution
   * environment. If the program is invoked from within the command line client to be submitted to a
   * cluster, this method returns the execution environment of this cluster.
   */
   ...
   *
   * This method sets the environment's default parallelism to given parameter, which defaults to
   * the value set via [[setDefaultLocalParallelism(Int)]].
   */
  def createLocalEnvironment(
      parallelism: Int = JavaEnv.getDefaultLocalParallelism): StreamExecutionEnvironment = {
  ...
  def createLocalEnvironment(
      parallelism: Int,
      configuration: Configuration): StreamExecutionEnvironment = {
    new StreamExecutionEnvironment(JavaEnv.createLocalEnvironment(parallelism, configuration))
  }

  /**
   * Creates a [[StreamExecutionEnvironment]] for local program execution that also starts the web
   * monitoring UI.
   *
   * The local execution environment will run the program in a multi-threaded fashion in the same
   * JVM as the environment was created in. It will use the parallelism specified in the parameter.
   *
   * If the configuration key 'rest.port' was set in the configuration, that particular port will be
   * used for the web UI. Otherwise, the default port (8081) will be used.
   *
   * @param config
   ...
   * is set explicitly via [[StreamExecutionEnvironment.setParallelism()]].
   *
   * @param host
