 ...
 * Connected streams are useful for cases where operations on one stream directly affect the
 * operations on the other stream, usually via shared state between the streams.
 *
 * An example for the use of connected streams would be to apply rules that change over time onto
...
class ConnectedStreams[IN1, IN2](javaStream: JavaCStream[IN1, IN2]) {
   ...
   *
   * The transformation consists of two separate functions, where the first one is called for each
   * element of the first connected stream, and the second one is called for each element of the
   * second connected stream.
   *
   * @param fun1
   ...
   *
   * The transformation calls [[CoMapFunction#map1]] for each element in the first stream and
   * [[CoMapFunction#map2]] for each element of the second stream.
   *
   * On can pass a subclass of [[org.apache.flink.streaming.api.functions.co.RichCoMapFunction]] to
   * gain access to the [[org.apache.flink.api.common.functions.RuntimeContext]] and to additional
   * life cycle methods.
   *
   * @param coMapper
  ...
  /**
   * Applies the given [[CoProcessFunction]] on the connected input streams, thereby creating a
   * transformed output stream.
   *
   * The function will be called for every element in the input streams and can produce zero or more
   * output elements. Contrary to the [[flatMap(CoFlatMapFunction)]] function, this function can
   * also query the time and set timers. When reacting to the firing of set timers the function can
   * directly emit elements and/or register yet more timers.
   *
  ...
  /**
   * Applies the given [[KeyedCoProcessFunction]] on the connected input keyed streams, thereby
   * creating a transformed output stream.
   *
   * The function will be called for every element in the input keyed streams and can produce zero
   * or more output elements. Contrary to the [[flatMap(CoFlatMapFunction)]] function, this function
   * can also query the time and set timers. When reacting to the firing of set timers the function
   * can directly emit elements and/or register yet more timers.
   *
   ...
   *
   * The transformation calls [[CoFlatMapFunction#flatMap1]] for each element in the first stream
   * and [[CoFlatMapFunction#flatMap2]] for each element of the second stream.
   *
   * On can pass a subclass of [[org.apache.flink.streaming.api.functions.co.RichCoFlatMapFunction]]
   * to gain access to the [[org.apache.flink.api.common.functions.RuntimeContext]] and to
  ...
  /**
   * Keys the two connected streams together. After this operation, all elements with the same key
   * from both streams will be sent to the same parallel instance of the transformation functions.
   *
   * @param keyPosition1
