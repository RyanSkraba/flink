 ...
 * programs that integrate with the Scala-specific [[DataStream]] API.
 *
...
trait StreamTableEnvironment extends TableEnvironment {
   ...
   * of the [[DataStream]]. If the outermost record's [[TypeInformation]] is a [[CompositeType]], it
   * will be flattened in the first level. [[TypeInformation]] that cannot be represented as one of
   * the listed [[DataTypes]] will be treated as a black-box
   * [[DataTypes.RAW(Class, TypeSerializer)]] type. Thus, composite nested fields will not be
   * accessible.
   *
   * Since the DataStream API does not support changelog processing natively, this method assumes
   * append-only/insert-only semantics during the stream-to-table conversion. Records of type
   * [[Row]] must describe [[RowKind.INSERT]] changes.
   *
   * By default, the stream record's timestamp and watermarks are not propagated unless explicitly
   * declared via [[fromDataStream(DataStream, Schema)]].
   *
   * @param dataStream
   *   The [[DataStream]] to be converted.
   * @tparam T
   ...
   * to a `CREATE TABLE` DDL in SQL and allows to:
   *
   ...
   * evaluates the [[RowKind]] flag that is contained in every record during runtime. The runtime
   * behavior is similar to that of a [[DynamicTableSource]].
   *
   ...
   *
   * By default, the stream record's timestamp and watermarks are not propagated unless explicitly
   * declared via [[fromChangelogStream(DataStream, Schema)]].
   *
   * @param dataStream
   ...
   *
   * See [[fromDataStream(DataStream, Schema)]] for more information and examples on how to declare
   * a [[Schema]].
   *
   * @param dataStream
   *   The changelog stream of [[Row]].
   ...
   * @param dataStream
   *   The [[DataStream]] out of which to create the view.
   * @tparam T
   ...
   *
   * Since the DataStream API does not support changelog processing natively, this method assumes
   * append-only/insert-only semantics during the table-to-stream conversion. The records of class
   * [[Row]] will always describe [[RowKind.INSERT]] changes. Updating tables are not supported by
   * this method and will produce an exception.
   *
   ...
   *
   * @param table
   *   The [[Table]] to convert. It must be insert-only.
   * @return
   ...
   *
   * See [[toDataStream(Table, AbstractDataType)]] for more information on how a Table} is
   * translated into a DataStream}.
   *
   ...
   *
   * The given [[DataType]] is used to configure the table runtime to convert columns and internal
   * data structures to the desired representation. The following example shows how to convert the
   * table columns into the fields of a POJO type.
   *
   * {{{
   ...
   *
   * Since the DataStream API does not support changelog processing natively, this method assumes
   * append-only/insert-only semantics during the table-to-stream conversion. Updating tables are
   ...
   * [[toChangelogStream(Table, Schema, ChangelogMode)]] to limit the kinds of changes (e.g. for
   * upsert mode).
   *
   * Note that the type system of the table ecosystem is richer than the one of the DataStream API.
   * The table runtime will make sure to properly serialize the output records to the first operator
   ...
   * upsert mode).
   *
   * The given [[Schema]] is used to configure the table runtime to convert columns and internal
   * data structures to the desired representation. The following example shows how to convert a
   * table column into a POJO type.
   *
   * {{{
   ...
   * all added statements together and then either submit them as one job or attach them to the
   * underlying [[StreamExecutionEnvironment]].
   *
   * @return
   *   statement set builder for the Scala-specific [[DataStream]] API
   */
   ...
   *   The [[DataStream]] to be converted.
   * @param fields
   *   The fields expressions to map original fields of the DataStream to the fields of the
   *   [[Table]].
   * @tparam T
   *   The type of the [[DataStream]].
   * @return
   *   The converted [[Table]].
   * @deprecated
   *   Use [[fromDataStream(DataStream, Schema)]] instead. In most cases,
   *   [[fromDataStream(DataStream)]] should already be sufficient. It integrates with the new type
   *   system and supports all kinds of [[DataTypes]] that the table runtime can consume. The
   *   semantics might be slightly different for raw and structured types.
   */
  @deprecated
   ...
   *   The type of the [[DataStream]].
   * @deprecated
   *   Use [[createTemporaryView(String, DataStream, Schema)]] instead. In most cases,
   *   [[createTemporaryView(String, DataStream)]] should already be sufficient. It integrates with
   *   the new type system and supports all kinds of [[DataTypes]] that the table runtime can
   ...
   *   The [[Table]] to convert.
   * @tparam T
   *   The type of the requested data type.
   * @return
   *   The converted [[DataStream]].
   * @deprecated
